    ...
    tokenize (string) {
        ...
        while (i < len) {
            ...
            if (/\s/.test(c)) {
                ...
                let m = /^(?:SELECT|FROM|WHERE|ORDER BY|LIMIT|GROUP BY|OFFSET|HAVING|EXPLAIN|WITH|WINDOW|VALUES|AS|USING|ON|LEFT|INNER|OVER|PARTITION BY|DISTINCT|FILTER|WITHIN GROUP|ASC|DESC|UNBOUNDED|PRECEDING|FOLLOWING|CURRENT ROW)\b/i.exec(ss);
                if (m) {
